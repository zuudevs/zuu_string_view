...
public:
    ...
    constexpr size_type find_first_of(const std::string& str, size_type pos = 0) const noexcept {
        return find_first_of_impl(str.data(), static_cast<size_type>(str.length()), pos);
    }
    ...
    constexpr size_type find_first_not_of(const CharT* str, size_type pos, size_type count) const noexcept {
        if (!str) {
			return npos;
		}
    ...
    constexpr size_type find_last_not_of(const CharT* str, size_type pos, size_type count) const noexcept {
        if (!str) {
			return npos;
		}
...
private:
    ...
    constexpr size_type find_first_of_impl(const CharT* str, size_type str_len, size_type pos) const noexcept {
        if (pos >= length_ || str_len == 0) return npos;
        if (str_len == 1) return find(str[0], pos);
    ...
    constexpr size_type find_first_not_of_impl(const CharT* str, size_type str_len, size_type pos) const noexcept {
    ...
    constexpr size_type find_last_of_impl(const CharT* str, size_type str_len, size_type pos) const noexcept {
    ...
    constexpr size_type find_last_not_of_impl(const CharT* str, size_type str_len, size_type pos) const noexcept {
        ...
        for (size_type i = pos + 1; i > 0; --i) {
            bool found = false;
            for (size_type j = 0; j < str_len; ++j) {
                if (data_[i - 1] == str[j]) { found = true; break; }
            }
            if (!found) return i - 1;
        }
        return npos;
